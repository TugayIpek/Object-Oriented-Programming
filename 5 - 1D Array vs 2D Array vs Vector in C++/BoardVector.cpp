...
namespace Tugay{
	int BoardVector::readFromFile(string text){		//reads from file and creats game board array
		string cursor;
		int gameboard[9][9];
		ifstream load;
		load.open(text);
	...
	bool BoardVector::move(char direction){		//takes direction and if move is possible, it makes move
		int temp = 0;		
	...
	void BoardVector::writeToFile(string text){		//takes games board array and a filename, writes to file its
	...
	void BoardVector::reset(){	//first filling function to board
		int number = 0;
		int last_index = 0;
	...
	void BoardVector::print(){		//prints board
		cout <<endl;
		for (auto i = 0; i < column; ++i)
		{
			for (auto k = 0; k < row; ++k)
	...
	void BoardVector::setSize(int c, int r){
		column = c;
		row = r;
		
		board.resize(column, vector<int>(row));
	}	
	int BoardVector::zero_y_coor(){		// ordinate of zero coordinate
		for (auto i = 0; i < column; ++i)
		{
	...
	int BoardVector::zero_x_coor(){		// axis of zero coordinate
		for (auto i = 0; i < column; ++i)
		{
		   	for (auto k = 0; k < row; ++k)		//founds place of zero
		   	{
	...
	bool BoardVector::isSolved(){		//checks the board from initial to goal
		int number = 0;
		int last_index = 0;
